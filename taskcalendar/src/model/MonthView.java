...
import adapter.CalendarDBAdapter;
...
public class MonthView extends TableLayout{
	...
	private void init(Context contxt)
	{
		...
		dns = DateNavService.getInstance();	
	...
	{
		CalendarDBAdapter db = new CalendarDBAdapter(context);
				...
				else	//day counter is in the current month
				{
					...
					else if(selected_day==day)
					{
						...
						for (int k = 0; k < entries.size(); k++) {
							...
							Log.d("MonthView", "Day: " + entries.get(k).getEday());
							
							test = Calendar.getInstance();//get current date and time's instance 
							test.clear(Calendar.HOUR);//remove the hour,minute,second and millisecond from the today variable
							test.clear(Calendar.MINUTE);
							test.clear(Calendar.SECOND);
							test.clear(Calendar.MILLISECOND);								
							test.set(Calendar.DAY_OF_MONTH, entries.get(k).getEday() );
							test.set(Calendar.MONTH, entries.get(k).getEmonth() );
							test.set(Calendar.YEAR, entries.get(k).getEyear() );
	...
	private OnClickListener dayClickedListener = new OnClickListener(){
		public void onClick(View v) {	
			...
			int navday = Integer.parseInt( v.getTag().toString() );
			
			dns.getDate().setDay( navday );	
			dns.getDate().setYear( cal.get( Calendar.YEAR ) );
			dns.getDate().setMonth(cal.get( Calendar.MONTH) );
